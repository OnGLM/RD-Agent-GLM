            ...
            try:
                ...
                while retrys < 5:
                    try:
                        ...
                        if isinstance(final_evaluation_dict, list):
                            # Filter out non-dictionary items
                            print("final_evaluation_dict返回数据有冗余需要进行处理")
                            final_evaluation_dict = [item for item in final_evaluation_dict if isinstance(item, dict)][0]
                        print("final_evaluation_dict:", final_evaluation_dict)
