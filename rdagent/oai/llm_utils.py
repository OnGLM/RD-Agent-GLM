        ...
        *,
        ...
        if self.cfg.use_llama2:
            ...
            if self.use_azure:
                ...
                self.chat_client = openai.OpenAI(api_key=self.chat_api_key, base_url="https://open.bigmodel.cn/api/paas/v4/")
                self.embedding_client = openai.OpenAI(api_key=self.embedding_api_key, base_url="https://open.bigmodel.cn/api/paas/v4/")
        ...
        if self.use_llama2:
            ...
            if json_mode:
                ...
                if resp.startswith('```json'):
                    resp = resp.strip()[len('```json'):].strip()
                if resp.endswith('```'):
                    resp = resp[:-len('```')].strip()
                ...
                try:
                    ...
                    logger.error(f"Problematic JSON data: {resp}")
                    raise
